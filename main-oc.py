...
def make_model(env, config):
    if config.model_name == 'maftrl-oc':
        model = AttentionSACM
    else:
    ...
    return model.init_from_env(env,
                               ...
                               max_tolerance=config.max_tolerance) if config.restore is False else model.init_from_save(
        load_dir / "model.pt", load_critic=True)


def get_reliable_obs(model, obs, recent_obs, validity):
    for e in range(config.n_rollout_threads):
        for a_i in range(model.nagents):
            if validity[a_i] == False and type(recent_obs[e][a_i]) != type(None):
...
if __name__ == '__main__':
    ...
    parser.add_argument("--max_tolerance", default=0.5, type=float)
